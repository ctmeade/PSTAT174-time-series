...
We remove the last 10 observations so that we may compare them against the forecasted data.
...
We make a few observations from this plot. First, there is a strong linear trend. The data is also yearly seasonal. Finally, variance slightly increases over time. As a result, we must transform the series to make it stationary.
...
To confirm the existence of a linear trend, we construct a linear model of the form $\hat{Y}=\beta_0+\beta_1 X_t$ and impose it onto the time series.
...
plot.ts(ts.log, ylab = "Log Atmospheric Carbon Dioxide (PPM)", main = "Log Atmospheric Carbon Dioxide vs. Time")
...
plot.ts(ts.log.1, ylab = "Diff. Log Atmospheric Carbon Dioxide (PPM)", main = "Diff. Log Atmospheric Carbon Dioxide vs. Time")
...
plot.ts(ts.log.1.12, ylab = "Twice Diff. Log Atmospheric Carbon Dioxide (PPM)", main = "Twice Diff. Log Atmospheric Carbon Dioxide vs. Time")
...
To confirm the stationarity of the series, we perform Kwiatkowski-Phillips-Schmidt-Shin (KPSS) test in `R` for the null hypothesis that the series is stationary.
...
* 1. $\text{SARIMA}\ (0,1,1)\ \text{x}\ (0,1,1)_{12}$
    * $\text{AICc}=-7671.1 \ \text{BIC}=-7653.05$
    * $\nabla_{12}\nabla X_t=(1-3.9B)(1-0.89B^{12})Z_t$
* 2. $\text{SARIMA}\ (4,1,0)\ \text{x}\ (0,1,1)_{12}$
    * $\text{AICc}=-7671.6 \ \text{BIC}=-7639.92$
    * $(1+0.37B.17B^2+0.13B^3+0.1B^4)\nabla_{12}\nabla X_t=(1-3.9B)(1-0.89B^{12})Z_t$
...
We perform diagnostic checking for the $\text{SARIMA}\ (0,1,1)\ \text{x}\ (0,1,1)_{12}$ model. Using the maximum likelihood estimation method, this model is given by:
...
Apart from a single outlier at $t=13$, the residuals appear to resemble white noise. We now plot the density of QQ-Plot of the residuals.
...
This model passes all adequacy at the $\alpha=0.05$ significance level.
...
We now check the adequacy of the second model. Just as before, we plot the residuals, density of the residuals, and the QQ-Plot of the residuals.

```{r, echo = F}
plot(model2$residuals, main="Model 2 Residuals")
```

Just as before, there is a large outlier at $t=13$. Besides this single point, the residuals resemble white noise.
...
Now that we have a final model, we may begin forecasting.
